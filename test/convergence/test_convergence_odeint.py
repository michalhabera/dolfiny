...
def generalised_alpha_1st_params():
    return {
        "euler_backward": {
            "param": {"alpha_f": 1.0, "alpha_m": 0.5, "gamma": 0.5},
            "order_expected": 1,
        },
        "crank_nicolson": {
            "param": {"alpha_f": 0.5, "alpha_m": 0.5, "gamma": 0.5},
            "order_expected": 2,
        },
...
def generalised_alpha_2nd_params():
    return {
        "newmark": {
            "param": {"alpha_f": 1.0, "alpha_m": 1.0, "gamma": 0.5, "beta": 0.25},
            "order_expected": 2,
        },
        "hht_rho_0.5": {
            "param": {"alpha_f": 2 / 3, "alpha_m": 1.0, "gamma": 5 / 6, "beta": 4 / 9},
            "order_expected": 2,
        },
        "wbz_rho_0.5": {
            "param": {"alpha_f": 1.0, "alpha_m": 4 / 3, "gamma": 5 / 6, "beta": 4 / 9},
            "order_expected": 2,
        },
        "generalised_alpha_rho_1.0": {"param": {"rho": 1.0}, "order_expected": 2},
        "generalised_alpha_rho_0.5": {"param": {"rho": 0.5}, "order_expected": 2},
...
@pytest.mark.parametrize(
    "jsonfile, odeint_m, closed_m",
    [
        ("test_odeint_linear_convergence.json", m.ode_1st_linear_odeint, m.ode_1st_linear_closed),
        (
            "test_odeint_nonlinear_convergence.json",
            m.ode_1st_nonlinear_odeint,
            m.ode_1st_nonlinear_closed,
        ),
        (
            "test_odeint_nonlinear_mdof_convergence.json",
            m.ode_1st_nonlinear_mdof_odeint,
            m.ode_2nd_nonlinear_closed,
        ),
    ],
)
def test_odeint_convergence(generalised_alpha_1st_params, jsonfile, odeint_m, closed_m):
    ...
    for method, info in generalised_alpha_1st_params.items():
        ...
        for N in (200, 400, 800, 1600):
            ...
            u_e, v_e = expected[:2]
            #
            l2["u"][N] = np.linalg.norm(u_c - u_e, 2) / np.linalg.norm(u_e, 2)
            l2["v"][N] = np.linalg.norm(v_c - v_e, 2) / np.linalg.norm(v_e, 2)
        ...
        for l2key, l2value in l2.items():
            ...
            k = 3
            x = np.log10(np.fromiter(l2value.keys(), dtype=float))
            y = np.log10(np.fromiter(l2value.values(), dtype=float))
            A = np.vstack([x[-k:], np.ones(k)]).T
            m = np.linalg.lstsq(A, y[-k:], rcond=None)[0][0]

            info["l2error"][l2key] = l2value
            info["order_measured"][l2key] = np.abs(m)

            assert np.greater(info["order_measured"][l2key], info["order_expected"] - 0.1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "test_odeint2_nonlinear_convergence.json",
            m.ode_2nd_nonlinear_odeint,
            m.ode_2nd_nonlinear_closed,
        ),
    ],
)
def test_odeint2_convergence(generalised_alpha_2nd_params, jsonfile, odeint_m, closed_m):
    ...
    for method, info in generalised_alpha_2nd_params.items():
        ...
        for N in (200, 400, 800, 1600):
            ...
            l2["u"][N] = np.linalg.norm(u_c - u_e, 2) / np.linalg.norm(u_e, 2)
            l2["v"][N] = np.linalg.norm(v_c - v_e, 2) / np.linalg.norm(v_e, 2)
            l2["a"][N] = np.linalg.norm(a_c - a_e, 2) / np.linalg.norm(a_e, 2)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "test_odeint_nonlinear_mdof_convergence.json",
            "ODEInt: 2x nonlinear 1st order ODEs, convergence",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "test_odeint2_nonlinear_convergence.json",
            "ODEInt2: nonlinear 2nd order ODE, convergence",
        ),
    ],
)
