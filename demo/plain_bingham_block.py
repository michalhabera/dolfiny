...
iR = 1.0; oR = 2.0; nR = 10*4; nT = 7*4; x0 = 0.0; y0 = 0.0; inner = 0; outer = 3
...
print("geometry subdomain area      = %4.3e (rel error = %4.3e)" % (area_msh, np.sqrt((area_msh-area_ana)**2/area_ana**2)))
ring_msh = df.fem.assemble_scalar( 1*ds(outer) )
ring_ana = 2.0 * np.pi * oR
print("geometry boundary outer ring = %4.3e (rel error = %4.3e)" % (ring_msh, np.sqrt((ring_msh-ring_ana)**2/ring_ana**2)))
ring_msh = df.fem.assemble_scalar( 1*ds(inner) )
ring_ana = 2.0 * np.pi * iR
print("geometry boundary inner ring = %4.3e (rel error = %4.3e)" % (ring_msh, np.sqrt((ring_msh-ring_ana)**2/ring_ana**2)))

# inner ring velocity
v_inner_ = lambda t=0.0, vt=v_inner_max, g=5, a=1, b=3: vt * 0.25*(np.tanh(g*(t-a))+1)*(np.tanh(-g*(t-b))+1)

# define helpers
n_vector_ = lambda x, r=iR: np.array([-x[0],-x[1]]) / r
t_vector_ = lambda x, r=iR: np.array([-x[1], x[0]]) / r

# boundary velocity as expression
v_vector_i_ = lambda x: t_vector_(x) * v_inner_(t=time.value)
v_vector_o_ = lambda x: np.zeros((mesh.geometry.dim, x.shape[1]))
...
for i in range(TS+1):
    ...
    array_shear_stress[i] = df.fem.assemble_scalar( rJ2(ufl.dev(T(v_,p_))) *ds(inner) ) / df.fem.assemble_scalar( 1.0*ds(inner) )
    array_shear_gammad[i] = df.fem.assemble_scalar( rJ2(ufl.dev(D(v_))   ) *ds(inner) ) / df.fem.assemble_scalar( 1.0*ds(inner) )
    array_tangent_velocity[i] = df.fem.assemble_scalar( ufl.sqrt(ufl.dot(v_,v_))*ds(inner) ) / df.fem.assemble_scalar( 1.0*ds(inner) )
    t_ = ufl.dot(ufl.dev(T(v_,p_)), n_vector)
    array_tangent_traction[i] = df.fem.assemble_scalar( ufl.sqrt(ufl.dot(t_,t_))*ds(inner) ) / df.fem.assemble_scalar( 1.0*ds(inner) )
...
mu_a = 0.5 * m; tau_a = c # slope m = 2*mu_a
