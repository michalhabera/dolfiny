...
def mesh_block3d_gmshapi(name="block3d", dx=1.0, dy=1.0, dz=1.0, nx=10, ny=10, nz=10, x0=0.0, y0=0.0, z0=0.0,
    ...
    if comm.rank == 0:
        ...
        e0 = gmsh.model.geo.extrude([(tdim - 3, p0)], dx, 0.0, 0.0,
                                    numElements=nele(nx), heights=prog(nx, px), recombine=do_quads)
        l0 = e0[1][1]
        # e1 = gmsh.model.geo.extrude([(tdim - 2, l0)], 0.0, dy, 0.0, recombine=do_quads)
        e1 = gmsh.model.geo.extrude([(tdim - 2, l0)], 0.0, dy, 0.0,
                                    numElements=nele(ny), heights=prog(ny, py), recombine=do_quads)
        s0 = e1[1][1]
        # e2 = gmsh.model.geo.extrude([(tdim - 1, s0)], 0.0, 0.0, dz, recombine=do_quads)
        _ = gmsh.model.geo.extrude([(tdim - 1, s0)], 0.0, 0.0, dz,
                                   numElements=nele(nz), heights=prog(nz, pz), recombine=do_quads)
