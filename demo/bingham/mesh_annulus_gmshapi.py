...
def mesh_annulus_gmshapi(
    ...
    if comm.rank == 0:
        ...
        s1 = gmsh.model.geo.revolve(
            [s0[0]], px, py, pz, ax, ay, az, angle=gmsh.pi / 2, numElements=[nT], recombine=do_quads
        )
        s2 = gmsh.model.geo.revolve(
            [s1[0]], px, py, pz, ax, ay, az, angle=gmsh.pi / 2, numElements=[nT], recombine=do_quads
        )
        s3 = gmsh.model.geo.revolve(
            [s2[0]], px, py, pz, ax, ay, az, angle=gmsh.pi / 2, numElements=[nT], recombine=do_quads
        )
        ...
        gmsh.model.addPhysicalGroup(
            tdim, [s0[1][1], s1[1][1], s2[1][1], s3[1][1]], domain
        )  # all sectors
        ...
        # boundary idx 4 and idx 5 obtained by inspection
        ring_inner = 1
        gmsh.model.addPhysicalGroup(
        ...
        gmsh.model.mesh.setTransfiniteCurve(
            l0, numNodes=nR, meshType="Progression", coef=progression
        )
