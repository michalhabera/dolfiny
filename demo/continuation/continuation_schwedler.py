...
import mesh_schwedler_gmshapi as mg
import numpy as np

import dolfiny

# Basic settings
name = "continuation_schwedler"  # see: https://doi.org/10.1016/j.finel.2006.04.007
comm = MPI.COMM_WORLD
...
gmsh_model, tdim = mg.mesh_schwedler_gmshapi(name, order=q)
...
# System properties
K = dolfinx.fem.Constant(mesh, scalar(6.4e5))  # axial stiffness [kN]
...
continuation.initialise(ds=0.1, λ=0.0)
...
if comm.size == 1:
    ...
    ax1.set_title("Schwedler dome", fontsize=12)
